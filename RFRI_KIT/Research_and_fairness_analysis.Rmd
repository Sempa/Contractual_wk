...
# library(readr)
final_dataset <- read_csv("final_dataset_new.csv")[-c(19, 27, 45, 122), ] %>%
  filter(`Which of the following best describes you?` != "NA") %>%
...
final_dataset <- final_dataset %>%
  ...
  rowwise() %>%
  mutate(
    `Meticulous research process` = mean(c(rpq10, rpq12, rpq3, rpq6, rpq20), na.rm = T),
    `Good mentoring` = mean(c(rpq5, rpq4), na.rm = T),
    `Adequate methods` = mean(c(rpq7, rpq18, rpq19), na.rm = T),
    `Transparent reporting` = mean(c(rpq1, rpq8, rpq14, rpq11), na.rm = T),
    `Ensuring reproducibility` = mean(c(rpq9, rpq15, rpq2), na.rm = T),
    `Open science` = mean(c(rpq16, rpq17, rpq13), na.rm = T)
  ) %>% # Research integrity scores
  mutate(
    `Engagement of local decision-makers` = mean(c(fpq2, fpq16, fpq15), na.rm = T),
    `Engagement of affected populations` = mean(c(fpq1, fpq14, fpq17), na.rm = T),
    `Partnership with local researchers` = mean(c(fpq3, fpq10, fpq4, fpq6), na.rm = T),
    `Fair agreements between study partners` = mean(c(fpq20, fpq19, fpq18, fpq17, fpq13), na.rm = T),
    `Local ethical approval` = mean(c(fpq8, fpq9), na.rm = T),
    `Respectful data collection` = mean(c(fpq5, fpq11, fpq12), na.rm = T)
  ) %>% # Research fairness score
  mutate(
    `responsible research Practices score` = ifelse(rpq == 0, NA, rpq),
    `fair research practices score` = ifelse(fpq == 0, NA, fpq)
  ) %>%
  mutate(`Research Discipline` = as.factor(case_when(
    `other_What discipline of global health based describes you:` == "Applied Health Research/Health Services Research & Health Technology Assessment" |
      `other_What discipline of global health based describes you:` == "Clinical Trial" ~ "Biomedical sciences",
    `other_What discipline of global health based describes you:` == "ENVIROMENTAL AND OCCUPATIONAL" |
      `other_What discipline of global health based describes you:` == "Environmental Health" |
      `other_What discipline of global health based describes you:` == "Environmental microbiology" |
      `other_What discipline of global health based describes you:` == "Foof and food safety" |
      `other_What discipline of global health based describes you:` == "policy" |
      `other_What discipline of global health based describes you:` == "public health" |
      `other_What discipline of global health based describes you:` == "Public Health" ~ "Environmental and occupational research",
    `What discipline of global health based describes you:` == "Biostatistics/Epidemiology" |
      `other_What discipline of global health based describes you:` == "EPI AND SOCIAL SCIENCES -MIXED METHODS" ~ "Biostatistics/Epidemiology",
    `What discipline of global health based describes you:` == "Biology/Medicine" |
      `other_What discipline of global health based describes you:` == "Microbiology" ~ "Biology/Medicine",
    `What discipline of global health based describes you:` == "Mathematics/Computer science" ~ "Mathematics/Computer science",
    `other_What discipline of global health based describes you:` == "psychology" |
      `other_What discipline of global health based describes you:` == "Public health and psychology" |

      `What discipline of global health based describes you:` == "Sociology/Anthropology" ~ "Sociology/Anthropology",
    `What discipline of global health based describes you:` == "Political sciences/Health economics" ~ "Political sciences/Health economics",
    `other_What discipline of global health based describes you:` == "health management" |
      `other_What discipline of global health based describes you:` == "Health Systems research" |
      `other_What discipline of global health based describes you:` == "Health policy and systems research with social sciences bias but with epi work also" |
      `other_What discipline of global health based describes you:` == "I am a research administrator." |
      `other_What discipline of global health based describes you:` == "primary health care, program implementation / evaluation" |
      `other_What discipline of global health based describes you:` == "Public Health/Behavioral Sciences" |
      `other_What discipline of global health based describes you:` == "Social sciences & public health" ~ "Health systems research",
    `What discipline of global health based describes you:` == "Prefer not to disclose" ~ "Prefer not to disclose",
    `What discipline of global health based describes you:` == "Unknown" ~ "Unknown"
  )))
  
  # x=final_dataset %>%
  # rowwise()%>%
  # mutate(new_var = mean(c(rpq10, rpq12, rpq3, rpq6, rpq20), na.rm = T),
  #   `Meticulous research process` = sum(c(rpq10, rpq12, rpq3, rpq6, rpq20), na.rm = T) / 5,
  #        `Good mentoring` = sum(c(rpq5, rpq4), na.rm = T) / 2,
  #        `Adequate methods` = sum(c(rpq7, rpq18, rpq19), na.rm = T) / 3,
  #        `Transparent reporting` = sum(c(rpq1, rpq8, rpq14, rpq11), na.rm = T) / 4,
  #        `Ensuring reproducibility` = sum(c(rpq9,rpq15, rpq2), na.rm = T) / 3,
  #        `Open science` = sum(c(rpq16, rpq17, rpq13), na.rm = T) / 3
  #        ) %>% #Research integrity scores
  # mutate(`Engagement of local decision-makers` = sum(c(fpq2, fpq16, fpq15), na.rm = T) / 3,
  #        `Engagement of affected populations` = sum(c(fpq1, fpq14, fpq17), na.rm = T) / 3,
  #        `Partnership with local researchers` = sum(c(fpq3, fpq10, fpq4, fpq6), na.rm = T) / 4,
  #        `Fair agreements between study partners` = sum(c(fpq20, fpq19, fpq18, fpq17, fpq13), na.rm = T) / 5,
  #        `Local ethical approval` = sum(c(fpq8, fpq9), na.rm = T) / 2,
  #        `Respectful data collection` = sum(c(fpq5, fpq11, fpq12), na.rm = T) / 3
  #        ) %>% #Research fairness score
...
```{r, message = FALSE, cache = TRUE, warning=FALSE, echo = FALSE, results='asis'}
dt01 <- final_dataset %>%
  # mutate(`Which country are you from?` = ifelse(`Which country are you from?`=='NA', 'Missing', `Which country are you from?`)) %>%
  dplyr::select(`Which country are you from?`, `Research Discipline`, #`What discipline of global health based describes you:`,
                `What is you gender?`, `How many years of involvement in research do you have?`, 
                `Which of the following best describes you?`, `What is your highest academic rank?`
                ) %>%
...
dt03 <- final_dataset %>%
  ...
  dplyr::select(`Meticulous research process`, `Good mentoring`, `Adequate methods`, 
                `Transparent reporting`, `Ensuring reproducibility`, `Open science`, 
                `Engagement of local decision-makers`, `Engagement of affected populations`,
                `Partnership with local researchers`, `Fair agreements between study partners`,
                `Local ethical approval`, `Respectful data collection`, location) %>%
  tbl_summary(by = location, 
            missing = "no",
            type = list(c('Meticulous research process', 'Good mentoring', 'Adequate methods', 
                'Transparent reporting', 'Ensuring reproducibility', 'Open science', 
                'Engagement of local decision-makers', 'Engagement of affected populations',
                'Partnership with local researchers', 'Fair agreements between study partners',
                'Local ethical approval', 'Respectful data collection') ~ 'continuous'),
            statistic = list(c('Meticulous research process', 'Good mentoring', 'Adequate methods', 
                'Transparent reporting', 'Ensuring reproducibility', 'Open science', 
                'Engagement of local decision-makers', 'Engagement of affected populations',
                'Partnership with local researchers', 'Fair agreements between study partners',
                'Local ethical approval', 'Respectful data collection') ~ "{mean} ({sd})")
            ) %>% 
  add_p() %>% #test = everything() ~ "wilcox.test"
  add_overall() %>%
  add_n() %>% 
  modify_header(label = "Category") %>% # update the column header
  bold_labels()
dt03

dt03_3 <- final_dataset %>%
  mutate(Location = as.factor(case_when(
    `Which of the following best describes you?` == "Researcher from the Global North" ~ "Global North",
    `Which of the following best describes you?` == "Researcher from the Global South" ~ "Global South" # ,
    # `Which of the following best describes you?` == '' ~ 'Global North',
  ...
  dplyr::select(
    `Meticulous research process`, `Good mentoring`, `Adequate methods`,
    `Transparent reporting`, `Ensuring reproducibility`, `Open science`,
    `Engagement of local decision-makers`, `Engagement of affected populations`,
    `Partnership with local researchers`, `Fair agreements between study partners`,
    `Local ethical approval`, `Respectful data collection`, Location
  ...
  gather(key = "X", "Mean score", 2:13) %>%
  mutate(
    category = rep(c("Questionable research practices", "Unfair research practices"), each = 12),
    `Behaviour categories` = c(
      "Meticulous research process", "Meticulous research process",
      "Good mentoring", "Good mentoring",
      "Adequate methods", "Adequate methods",
      "Transparent reporting", "Transparent reporting",
      "Ensuring reproducibility", "Ensuring reproducibility",
      "Open science", "Open science",
      "Engagement of local decision-makers", "Engagement of local decision-makers",
      "Engagement of affected populations", "Engagement of affected populations",
      "Partnership with local researchers", "Partnership with local researchers",
      "Fair agreements between study partners", "Fair agreements between study partners",
      "Local ethical approval", "Local ethical approval",
      "Respectful data collection", "Respectful data collection"
    )
  )
positions = c(
  "Respectful data collection", "Local ethical approval","Fair agreements between study partners","Partnership with local researchers", "Engagement of affected populations","Engagement of local decision-makers","Open science","Ensuring reproducibility", "Transparent reporting","Adequate methods","Good mentoring", "Meticulous research process")
dt03_4 <- dt03_3 %>%
  ggplot(aes(x = `Behaviour categories`, y = `Mean score`, fill = Location )) +
  geom_bar(stat="identity", color="black", position = position_dodge()) +
scale_fill_manual(values=c("#999999", "#E69F00"))+ scale_x_discrete(limits = positions)+
geom_text(aes(label = `Mean score`), size = 4, position = position_dodge(.9), fontface = 12)+#, position = position) +
  theme(
...
# dt03_4
jpeg('figure12.jpeg', units = "in", width = 12, height = 8, res = 300)
dt03_4
dev.off()
```
...
dt03_5 <- final_dataset %>%
  mutate(`Research field` = as.factor(case_when(
    `What discipline of global health based describes you:` == 'Biology/Medicine' | 
      `What discipline of global health based describes you:` == 'Biostatistics/Epidemiology' | 
      `What discipline of global health based describes you:` == 'Mathematics/Computer science' | 
      `other_What discipline of global health based describes you:` == 'Applied Health Research/Health Services Research & Health Technology Assessment' |
`other_What discipline of global health based describes you:` == 'Clinical Trial'|
`other_What discipline of global health based describes you:` == 'ENVIROMENTAL AND OCCUPATIONAL'|
`other_What discipline of global health based describes you:` == 'Environmental Health'|
`other_What discipline of global health based describes you:` == 'Environmental microbiology'|
`other_What discipline of global health based describes you:` == 'EPI AND SOCIAL SCIENCES -MIXED METHODS'|
`other_What discipline of global health based describes you:` == 'Foof and food safety'|
`other_What discipline of global health based describes you:` == 'Microbiology'|
`other_What discipline of global health based describes you:` == 'psychology'|
`other_What discipline of global health based describes you:` == 'Public health and psychology'|
`other_What discipline of global health based describes you:` == 'public health'|
`other_What discipline of global health based describes you:` == 'Public Health' ~ 'Biomedical',
    `What discipline of global health based describes you:` == 'Sociology/Anthropology' |
  `What discipline of global health based describes you:` == 'Political sciences/Health economics' |
  `other_What discipline of global health based describes you:` == 'health management'|
`other_What discipline of global health based describes you:` == 'Health Systems research'|
`other_What discipline of global health based describes you:` == 'Health policy and systems research with social sciences bias but with epi work also'|
`other_What discipline of global health based describes you:` == 'I am a research administrator.'|
...
`other_What discipline of global health based describes you:` == 'policy'|
`other_What discipline of global health based describes you:` == 'Public Health/Behavioral Sciences'|
`other_What discipline of global health based describes you:` == 'Social sciences & public health' ~ 'Non biomedical'
  ))) %>%
  dplyr::select(`Meticulous research process`, `Good mentoring`, `Adequate methods`, 
                `Transparent reporting`, `Ensuring reproducibility`, `Open science`, 
                `Engagement of local decision-makers`, `Engagement of affected populations`,
                `Partnership with local researchers`, `Fair agreements between study partners`,
                `Local ethical approval`, `Respectful data collection`, `Research field`) %>%
  tbl_summary(by = `Research field`, 
            missing = "no",
            type = list(c('Meticulous research process', 'Good mentoring', 'Adequate methods', 
                'Transparent reporting', 'Ensuring reproducibility', 'Open science', 
                'Engagement of local decision-makers', 'Engagement of affected populations',
...
x_sum <- dt03_6 %>% 
  group_by(`Research field`, category) %>% 
  summarise(overal_mean = mean(`Mean score`), sd_overal = sd(`Mean score`))
x_sum_1 <- dt03_6 %>% 
  group_by(`Behaviour categories`) %>% 
  summarise(overal_mean = mean(`Mean score`), sd_overal = sd(`Mean score`))

positions = c("Meticulous research process", "Good mentoring", "Adequate methods",
      "Transparent reporting", "Ensuring reproducibility", "Open science",
      "Engagement of local decision-makers", "Engagement of affected populations",
      "Partnership with local researchers", "Fair agreements between study partners",
      "Local ethical approval","Respectful data collection")
dt03_7 <- dt03_6 %>%
  ggplot(aes(x = `Behaviour categories`, y = `Mean score`, fill = `Research field` )) +
  geom_bar(stat="identity", color="black", position = position_dodge()) +
scale_fill_manual(values=c("#999999", "#E69F00"))+ scale_x_discrete(limits = positions)+
geom_text(aes(label = `Mean score`), size = 4, position = position_dodge(.9), fontface = 12)+#, position = position) +
  theme(
  ...
  coord_flip() +
  scale_fill_manual("Research field",
                    values = c(
                      "Biomedical" = "#E69F00",
                      "Non biomedical" = "#999999"
                    )) +
  guides(fill = guide_legend(reverse = TRUE))
# dt03_7
jpeg('figure2.jpeg', units = "in", width = 12, height = 8, res = 300)
dt03_7
dev.off()
```
...
dt03
# dt03_1 <- dt03 %>%
#   filter(!is.na(`responsible research Practices score`)) %>%
#   dplyr::select(location, `responsible research Practices score`) %>%
#   group_by(location) %>%
#   summarise(across(where(is.numeric), list(mean = mean, sum = sum, sd = sd)))
# dt03_2 <- dt03 %>%
#   filter(!is.na(`fair research practices score`)) %>%
#   dplyr::select(location, `fair research practices score`) %>%
#   group_by(location) %>%
#   summarise(across(where(is.numeric), list(mean = mean, sum = sum, sd = sd)))
```
...
dat <- final_dataset %>%
  ...
  drop_na(c('Which country are you from?', 'Research Discipline', 'What is you gender?', 'How many years of involvement in research do you have?', 'Which of the following best describes you?', 'What is your highest academic rank?', 'responsible research Practices score', 'fair research practices score', 'Location')) %>%
  dplyr::select(`Which country are you from?`, `Research Discipline`, `What is you gender?`, `How many years of involvement in research do you have?`, Location, `What is your highest academic rank?`, `responsible research Practices score`, `fair research practices score`, ) 
univariate_table <- dat %>%
  ...
  mutate(
    ...
    discipline_global_health = as.factor(`Research Discipline`)
  ) %>%
...
finalmodel_dataset <- dat %>%
  ...
  dplyr::select(rpq_score, Location, `What is your highest academic rank?`, `How many years of involvement in research do you have?`, `What is you gender?`, `Which country are you from?`, `Research Discipline`) %>%
...
step.model <- stepAIC(model, direction = "both", 
                      trace = F)
# summary(step.model)
# glm(formula = as.formula('rpq_score~ Location + accademic_rank'),
#               data = finalmodel_dataset,
#               family = gaussian,
#               ) %>%
...
mv_model <- glm(formula = as.formula('rpq_score~ Location + accademic_rank + year_involment_research + gender + country_origin + discipline_global_health'),
...
tbl_merge_ex1 <-
  tbl_merge(
    tbls = list(univariate_table_1, mv_model),
...
univariate_table_2 <- dat %>%
  ...
  dplyr::select(fpq_score, Location, `What is your highest academic rank?`, `How many years of involvement in research do you have?`, `What is you gender?`, `Which country are you from?`, `Research Discipline`) %>%
  mutate(accademic_rank = as.factor(`What is your highest academic rank?`),
         year_involment_research = as.factor(`How many years of involvement in research do you have?`),
         gender = as.factor(`What is you gender?`),
         country_origin = as.factor(`Which country are you from?`),
...
finalmodel_dataset3 <- dat %>%
  ...
  dplyr::select(fpq_score, Location, accademic_rank, year_involment_research, gender, country_origin, discipline_global_health)
mv_model2 <- glm(formula = as.formula('fpq_score~ Location + accademic_rank + year_involment_research + gender + country_origin + discipline_global_health'),
...
tbl_merge_ex2 <-
  tbl_merge(
    tbls = list(univariate_table_3, mv_model2),
    tab_spanner = c("**Univariate**", "**Multivariate**")
  )
tbl_merge_ex2
