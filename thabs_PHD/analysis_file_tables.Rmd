...
table_1 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `log10 viral load`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils) %>% #`Opportunistic infection present`, 
tbl_summary(#table_1,
            missing = "no",
            by = `Gender, n(%)`,
...
dt03 <- final_dataset %>%
  ...
  pivot_longer(cols=c(`Concomitant therapy (1)`, `Concomitant therapy (2)`, `Concomitant therapy (3)`, `Concomitant therapy (4)`, `Concomitant therapy (5)`, `Concomitant therapy (6)`, `Concomitant therapy (7)`, `Concomitant therapy (8)`, `Concomitant therapy (9)`, `Concomitant therapy (10)`),
                    ...
                    values_to='drug2') %>%
  filter(drug2 != 'NA')
write.csv(dt03, 'list of drugs.csv')
dt04 <- dt03 %>%
  mutate(id_1 = paste(id, drug, sep = '_')) %>%
  left_join(read_delim("list of drugs - updated.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE) %>%
      mutate(id_1 = paste(id, drug, sep = '_')) %>%
      dplyr::select(id_1, ART), by = 'id_1') %>%
  dplyr::select(id, ART) %>%
  mutate(ART = ifelse(!is.na(ART), 1, 0)) %>%
  group_by(id) %>%
    mutate(ART_exposure = max(ART)) %>%
    ungroup() %>%
  # mutate(ART = ifelse(ART == 1, "ART", "no ART")) %>%
  distinct( id, .keep_all = T) %>%
  dplyr::select(id, ART_exposure)
box_plot_CD4 <- final_dataset %>%
  dplyr::select(id = `Hospital folder number`, `CD4 count` = `Total CD4 count...10`) %>%
  left_join(dt04, by = 'id') %>%
  mutate(`ART exposure` = as.factor(ifelse(is.na(ART_exposure), 'no ART',
                                         ifelse(ART_exposure == 1, 'ART',
                                                ifelse(ART_exposure == 0, 'no ART', ART_exposure))))) %>%
  mutate(`CD4 count` = as.numeric(`CD4 count`)) %>%
  ggplot(aes(x= `ART exposure`, y = `CD4 count`)) +
geom_boxplot() +
  theme(
    text = element_text(size = 20),
    plot.title = element_text(hjust = 0.5),
...
jpeg('boxplot_CD4.png', units = "in", width = 8, height = 9, res = 300)
...
table_2 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `Viral load (log10 Copies/mL)` = `log10 viral load`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `Sodium mmol/L` = Sodium, `Potassium mmol/L` = Potassium, `Haemoglobin g/dL` = Haemoglobin, 
    ...
    `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection` = `Opportunistic infection present`, 
                `3 followup_patientstatus`, `6 followup_patientstatus`, `12 followup_patientstatus`, `Addisons disease`) %>%
...
table_2_1 <- final_dataset %>%
  ...
  dplyr::select(
   ...
   `3 followup_patientstatus`, `6 followup_patientstatus`, `12 followup_patientstatus`#, `total AI` = total_AI
                ) %>%
        tbl_summary( # table_1,
          missing = "no",
          by = `AI`,
...
model2 <- survival::coxph(Surv(time = ttdeath, event = mortality) ~ AI + CD4_count + Lymphocyte_count + Potassium ,#+ Gender,
...
model2
model3 <- glm(mortality ~ AI + CD4_count + Lymphocyte_count + Potassium ,#+ Gender,
...
model1 <- glm(AI ~ Potassium + Neutrophils + CD4_count,#ACTH + Random_cortisol + Lymphocyte_count,#ACTH + Stimulated_cortisol + BP_diastolic,
                                                 family = binomial(link = "logit"),
