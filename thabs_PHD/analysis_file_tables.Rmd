...
cleaned_mortality_dataset <- table_3 %>%
  # Filter to look at clinical characteristics only among patients who died
  filter(mortality == "yes" | mortality == 1) %>% 
  mutate(
    Ethnicity = as.character(Ethnicity),
    HIV_duration = as.numeric(as.Date(`Date of enrolment in study`, format='%m/%d/%Y') - 
                                as.Date(`HIV, when diagnosed`, format='%m/%d/%Y')),
    `Addisons disease` = forcats::fct_rev(as.factor(`Addisons disease`)),
    
    # Branching logic fix: Convert "Not applicable" to NA so it's excluded from denominator
    `Loss of axillary and pubic hair, if female` = case_when(
      `Loss of axillary and pubic hair, if female` == "Not applicable" ~ NA_character_,
      TRUE ~ as.character(`Loss of axillary and pubic hair, if female`)
    )
...
# 2. GENERATE AND STACK SUB-TABLES IN A SINGLE STRUCTURAL PIPE
table_4 <- tbl_stack(
  list(
    ...
    # Sub-table B: History Subheading Variables (Includes your fixed branching variable)
    cleaned_mortality_dataset %>%
      dplyr::select(
        `Addisons disease`,
      ...
      tbl_summary(by = `Addisons disease`, missing = "no") %>% 
      add_p(),
    
    # Sub-table C: Clinical Features Subheading Variables
    cleaned_mortality_dataset %>%
      dplyr::select(
        `Addisons disease`,
    ...
    # Sub-table D: Investigations Subheading Variables
    cleaned_mortality_dataset %>%
      dplyr::select(
        `Addisons disease`,
  ...
  # 3. GLOBAL MULTIPLE-TESTING FALSE DISCOVERY RATE ADJUSTMENT (B-H Method)
  modify_table_body(
    ~ .x %>%
      mutate(p.value = p.adjust(p.value, method = "BH"))
  ) %>%
  
  # 4. INLINE LABELS & FORMATTING ENGINE CONVERSION
  modify_header(
    label = "**Variable**",
    p.value = "**p-value**"
  ) %>%
  ...
  as_gt() %>%
  gt::tab_header(
    title = "Table 3: Comparisons of the clinical characteristics of patients who died with AI versus those without"
  ) %>%
  gt::tab_source_note(
    source_note = "*Difference in the basal, compared to the stimulated cortisol, expressed as a percentage basal cortisol p-value is a comparison of clinical characteristics of patients with AI versus those without AI, among all those who had died"
  )

# Display final formatted object
table_4
...
library(ggplot2)
library(dplyr)
library(ggsurvplot) # for initial data extraction
