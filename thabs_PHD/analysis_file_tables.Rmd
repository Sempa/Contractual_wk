...
final_dataset <- read_excel("hypo_alldata_wide_labels.xls") %>%
  ...
  mutate(addisons_disease = ifelse(as.numeric(`Random cortisol result`) < 500, 1, 0),
         ...
         PAI = ifelse(`Random cortisol result`<=274 & (is.na(`Synacthen: 0 minute cortisol result`) | is.na(`Synacthen: 30 minute cortisol result`)) & (as.numeric(`ACTH result`) >= 64.7 & !is.na(`ACTH result`)) & PAI %in% c(0, NA), 1, PAI),
...
# `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`
  ...
  modify_header(label = "Variable") %>% # update the column header
  bold_labels()  %>%
  modify_caption('Table 3 (article): Should compare Total AI with Non-AI patients')
...
PTB, EPTB,  `GE/c diff`, EPTB, PTB, AI
  ...
  modify_header(label = "Variable") %>% # update the column header
  bold_labels()  %>%
  modify_caption('Table 2 (article): Should compare Total PAI with SAI patients')
