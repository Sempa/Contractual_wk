...
# Step 3: Generate initial plot to extract curve data
km_plot2 <- ggsurvplot(km_fit2, data = dt04_adj)

# Step 4: Extract and jitter data
plot_data <- km_plot2$plot$data %>%
  mutate(time_jittered = ifelse(strata == "AI", time + 0.2, time))

# Step 5: Plot with adjusted axes
km_fit3 <- ggplot(plot_data, aes(x = time_jittered, y = surv, color = strata)) +
  geom_step(size = 1.2) +
  geom_point(size = 1.5) +
...
ggsave("KM_jittered_plot.png", km_fit3, width = 8, height = 6, dpi = 300)
