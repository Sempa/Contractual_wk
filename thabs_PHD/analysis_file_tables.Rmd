...
table_2 <- tbl_stack(
  list(
    ...
    cleaned_dataset %>% 
      ...
      tbl_summary(by = `Addisons disease`, missing = "no", type = all_continuous() ~ "continuous", statistic = list(all_continuous() ~ "{median} ({p25}, {p75})"), digits = list(all_categorical() ~ c(0, 1), all_continuous() ~ c(1, 1))),
...
data_to_impute <- bind_cols(data_reduced, dt %>% dplyr::select(Tiredness, Loss_of_consciousness, Diarrhoea, )) #%>%
  # dplyr::select(mortality, selected_models$term)
matched_terms <- univariate_table$table_body$variable %>% unique()
# 1. Subset the data first so we know exactly what variables are going into the regression
regression_data <- data_to_impute %>% 
  dplyr::select(mortality, any_of(matched_terms)) %>% 
...
merged_cox_report
# =====================================================================
# 1. CROSS-VALIDATED ELASTIC NET FOR LOGISTIC REGRESSION
# =====================================================================
lasso_results <- lapply(imp_list, function(dat) {
...
# =====================================================================
# 2. VARIABLE SELECTION STABILITY STAGE
# =====================================================================
selected_vars <- lapply(lasso_results, function(fit) {
  beta <- coef(fit, s = "lambda.min")
  tibble::tibble(
...
# Create the logistic formula string allocation
final_formula <- as.formula(
  paste("mortality ~", paste(stable_vars, collapse = " + "))
)

# This loops through your clean list of dataframes where 'mortality' actually exists
logistic_fit_list <- lapply(imp_list, function(dat) {
  stats::glm(
    mortality ~ ACTH + Opiates + Random_cortisol,
    data = dat,
...
# =====================================================================
# 2. CONVERT TO MIRA OBJECT FOR GTSUMMARY COMPATIBILITY
# =====================================================================
# Convert the list of fits into a mice-compatible 'mira' object
mira_logistic_fits <- mice::as.mira(logistic_fit_list)

# =====================================================================
# 3. BUILD AND RENDER THE POOLED LOGISTIC TABLE
# =====================================================================
pooled_logistic_table <- tbl_regression(
  mira_logistic_fits,                           
  exponentiate = TRUE,                 # Exponentiate coefficients to get Odds Ratios (OR)
  label = list(
    ACTH            ~ "ACTH Level",
    Opiates         ~ "Opiate Exposure",
    Random_cortisol ~ "Random Cortisol"
  )
) %>%
  modify_header(
    label    = "**Clinical Feature**",
    estimate = "**Pooled OR**",       # Relabeled from HR to OR for Logistic
    ci       = "**95% CI**",
    p.value  = "**p-value**"
  ) %>%
  bold_labels()

# # 4. Print safely to the console bypassing the HTML viewer engine
# pooled_logistic_table

# =====================================================================
# 1. PURGE INVALID UTF-8 BYTES FROM UNDERLYING TABLE BODIES
# =====================================================================

# Clean the univariate table internal data
univariate_table$table_body <- univariate_table$table_body %>%
  mutate(across(where(is.character), ~ iconv(.x, to = "UTF-8", sub = "")))

# Clean the multivariable table internal data
pooled_table_logistic$table_body <- pooled_table_logistic$table_body %>%
  mutate(across(where(is.character), ~ iconv(.x, to = "UTF-8", sub = "")))

# =====================================================================
# 2. MERGE THE SANITIZED TABLES
# =====================================================================
merged_logistic_report <- tbl_merge(
  tbls        = list(univariate_table, pooled_table_logistic),
  tab_spanner = c("**Univariable Analysis**", "**Multivariable (Pooled) Model**")
) %>%
  # Safely recode the clean strings now
  modify_table_body(~ .x %>% dplyr::mutate(
    label = dplyr::case_when(
      variable == "ACTH" ~ "ACTH Level",
      variable == "Opiates" ~ "Opiate Exposure",
...
<!--   ) %>% -->
<!--   dplyr::select(`Age at enrolment`, Gender, Ethnicity, `Duration of current illness`, `log10 viral load`, `Total CD4 count`, Sodium, Potassium, Haemoglobin, `White cell count`, `Lymphocyte count`, Neutrophils, `Addisons disease`, mortality, ttdeath, `Random cortisol result`, `Synacthen: 0 minute cortisol result`, `Synacthen: 30 minute cortisol result`, `ACTH result`, incremental_cortisol, incremental_cortisol_percent_increase, `BP (systolic)`, `BP (diastolic)`, discharged, -->
<!--                 `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Weight loss`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection` = `Opportunistic infection present`, -->
