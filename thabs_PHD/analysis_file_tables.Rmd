...
final_dataset <- final_dataset %>%
  ...
  left_join(read_csv("opportunistic infections_549 - TB.csv") %>%
...
table_12 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `log10 viral load`, Tuberculosis, PTB, EPTB, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `CD4 counts`, `ART exposure`, `Kidney medication`, `TB medication`, `Fungal medication`
    ) %>%# ,
tbl_summary(#table_1,
            missing = "no",
            by = `CD4 counts`,
...
table_4 <- table_3 %>%
  ...
  dplyr::select(
    `Age at enrolment, median (IQR) (years)` = Age_at_enrolment, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = Duration_of_current_illness, `Random cortisol`, `Basal cortisol`, `Stimulated cortisol`, `ACTH`, `Weight loss`, `Viral load (log10 Copies/mL)`, Tuberculosis, PTB, EPTB,  `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count` = `CD4 count`, `White cell count X109` = White_cell_count, `Lymphocyte count X109` = Lymphocyte_count, Neutrophils, `Sodium mmol/L` = Sodium, `Potassium mmol/L`, `Haemoglobin g/dL` = Haemoglobin, 
...
dt04 <- table_3 %>%
  ...
  dplyr::select(
    ...
    `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection`, 
    `ART exposure`, `Kidney medication`, incremental_cortisol, flag) %>% #`Addisons disease` = Addisons_disease
  # mutate(`Addisons disease` = forcats::fct_rev(`Addisons disease`)) %>%
  tbl_summary(#table_1,
    missing = "no",
...
x <- survival::coxph(Surv(time = t, event = event) ~ AI,
                  data = completeData_mort %>%
                      mutate(t = ifelse(ttdeath >3, 3, ttdeath),
                             event = ifelse(ttdeath == 3, 1, mortality)) %>%
...
summary(survival::coxph(Surv(time = t, event = event) ~ AI,
                  data = completeData_mort %>%
                      mutate(t = ifelse(ttdeath >6, 6, ttdeath),
                             event = ifelse(ttdeath == 6, 1, mortality)) %>%
                      mutate(Addisons = as.factor(ifelse(as.character(AI) == 'AI', "Yes", "No"))) %>%
                      dplyr::select(mortality, event, ttdeath, t, AI)
))$coefficients[5]
...
summary(survival::coxph(Surv(time = t, event = event) ~ AI,
                  data = completeData_mort %>%
                      mutate(t = ifelse(ttdeath >12, 12, ttdeath),
                             event = ifelse(ttdeath == 12, 1, mortality)) %>%
                      mutate(Addisons = as.factor(ifelse(as.character(AI) == 'AI', "Yes", "No"))) %>%
                      dplyr::select(mortality, event, ttdeath, t, AI)
))$coefficients[5]
...
ggsurvplot(
  ...
  legend.labs =
    c("No AI", "AI"),    # Change legend labels
...
ggpar(survminer::ggsurvplot(km_fit,
  ...
  legend.labs =
    c("No AI", "AI"),
  pval = TRUE,
  xlab = "Time in months",              # Add p-value
  risk.table = TRUE,        # Add risk table
  # risk.table.col = "strata",# Risk table color by groups
  # legend.labs =
