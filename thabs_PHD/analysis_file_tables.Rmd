...
# x=final_dataset %>% filter(`Random cortisol result`< 400 & (as.numeric(`Synacthen: 30 minute cortisol result`) < 274 | is.na(as.numeric(`Synacthen: 30 minute cortisol result`)))) %>% select(UIN...1, `Random cortisol result`, `Synacthen: 30 minute cortisol result`)
```
...
dt04_adj <- dt04 %>%
  mutate(strata = ifelse(Addisons_disease == 2, "AI", 
         ...
         strata_2 = ifelse(
           record_id %in% c(53, 70, 212, 229, 236, 275, 322, 477, 488) & strata == "AI", "Not treated", ifelse(strata == "AI", "Treated", "")))
...
km_fit2_v2 <- survfit(Surv(ttdeath, mortality) ~ strata_2, data = dt04_adj)

# 3. Overall Log-Rank P-value
overall_diff <- survdiff(Surv(ttdeath, mortality) ~ strata_2, data = dt04_adj)
overall_p_val <- 1 - pchisq(overall_diff$chisq, df = length(overall_diff$n) - 1)
overall_p_txt <- ifelse(overall_p_val < 0.001, "Overall p < 0.001", 
                        paste0("Overall p = ", sprintf("%.3f", overall_p_val)))
...
# 5. Extract plot data with horizontal jitter
km_plot2_v2 <- ggsurvplot(km_fit2_v2, data = dt04_adj)
...
stable_vars <- selected_vars |>
  ...
  table() |>
  as.data.frame() |>
  rename(variable = Var1, freq = Freq) |>
  filter(freq >= 0.6 * length(imp_list)) |>
  pull(variable)

# Ensure 'date_diff' or other crucial tracking elements remain isolated if targeted
# stable_vars <- unique(c(stable_vars, "date_diff")) 

# =====================================================================
# 2. FIT MULTIVARIABLE MODELS ACROSS IMPUTED DATASETS (POOLED MICE)
# =====================================================================

# Dynamically construct the formula string using the stable variables
dynamic_mv_formula <- as.formula(
  paste("mortality ~", paste(c(stable_vars, "Addisons_disease"), collapse = " + "))
)
