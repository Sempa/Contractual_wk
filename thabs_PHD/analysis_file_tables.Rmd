...
table_2 <- final_dataset %>%
  ...
  # filter(AI != "") %>%
  dplyr::select(`Addisons disease`, `Age at enrolment, median (IQR) (years)` = `Age at enrolment`,
`Gender, n(%)` = `Gender`,
...
Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea,
Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `BP (diastolic)`,
`Heart rate`, Hypotension, `Any postural drop in blood pressure`, `Presence of anaemia`, 
`Viral load (log10 Copies/mL)` = `log10 viral load`, PTB, EPTB, `Cryptococcus neoformans`,
Pneumonia, HepB, Candida, `Kaposis sarcoma`, PCP, HSV, `GE/c diff`, `Total CD4 count`,
`White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`,
Neutrophils, `Sodium mmol/L` = Sodium, `Potassium mmol/L` = Potassium, 
`Haemoglobin g/dL` = Haemoglobin, mortality, Fluconazole, Opiates, `ART exposure`
...
# `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`
  ...
  tbl_summary( # table_1,
    ...
    type = list(c("Duration of current illness, median (IQR) (days)", #"Random cortisol", "Basal cortisol", "Stimulated cortisol", "ACTH",
                  # "BP (systolic)", 
                  "BP (diastolic)", "Potassium mmol/L", "Age at enrolment, median (IQR) (years)", "Viral load (log10 Copies/mL)", "Total CD4 count", "White cell count X109", "Haemoglobin g/dL", "Sodium mmol/L", "Heart rate", "Lymphocyte count X109", "Neutrophils") ~ "continuous"),#
...
table_2 <- table_2 %>%
  ...
  as_gt() %>%
  gt::tab_header(
    title = 'Table 3 (article): Should compare Total AI with Non-AI patients'
  )
...
`Loss of axillary and pubic hair, if female`, `BP (systolic)`, `BP (diastolic)`,
`Heart rate`, Hypotension, `Any postural drop in blood pressure`, `Presence of anaemia`,
`Sodium mmol/L` = Sodium, `Potassium mmol/L`, `Haemoglobin g/dL` = Haemoglobin,
`Syphilis`, Candida, HepB, HSV, Pneumonia, PCP, PTB, EPTB, `Cryptococcus neoformans`,
`Total CD4 count` = `CD4 count`, `Viral load (log10 Copies/mL)`, `Random cortisol`,
`Basal cortisol`, `Stimulated cortisol`, incremental_cortisol, 
incremental_cortisol_percent_increase
    ) %>%
  # mutate(`Addisons disease` = forcats::fct_rev(`Addisons disease`)) %>%
tbl_summary(#table_1,
...
table_4 <- table_4 %>%
  ...
  as_gt() %>%
  gt::tab_header(
    title = 'Table 4 (article): Mortality: Non-AI vs AI'
  )
