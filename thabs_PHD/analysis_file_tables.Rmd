...
box_plot_1 <- ggboxplot(
  dt02 %>% filter(as.character(Group) != 'ACTH') %>%
    mutate(`AI status` = forcats::fct_rev(`AI status`)),
  x = 'Group', y = 'result',
  fill = "AI status", 
  palette = c("Adrenal insufficiency" = "gray40", "No Adrenal insufficiency" = "white"),
...
for (i in seq_along(time_points)) {
  # Survival probabilities at the specific time point
  surv_probs <- summary(km_fit2, times = time_points[i])$surv
  
  # Perform a pairwise log-rank test for survival differences
  surv_diff <- survdiff(Surv(time = ttdeath, event = mortality) ~ strata, 
                        data = dt04 %>%
                          mutate(strata = ifelse(strata != "No AI", 'AI', strata )), 
                        subset = (ttdeath >= time_points[i]))
...
dynamic_mv_formula <- as.formula(
  paste("mortality ~", paste(stable_vars, collapse = " + "))
)
