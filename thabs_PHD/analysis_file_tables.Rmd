...
model4 <- survival::coxph(Surv(time = ttdeath, event = mortality) ~ Viral_load + Cryptococcus_neoformans + Kidney_medication + Diarrhoea + incremental_cortisol,
                          data = completeData_mort %>%
                            # filter(as.character(Gender) != 'Unknown') %>%
                            mutate(Basal_cortisol = Basal_cortisol/50)
                          ) %>%#AI_recoded,# # + Dizziness,
...
model5 <- glm(mortality ~ Viral_load + Cryptococcus_neoformans + Kidney_medication + Diarrhoea + incremental_cortisol,
              family = binomial(link = "logit"),
              data = completeData_mort %>%
                mutate(Basal_cortisol = Basal_cortisol/50)
              ) %>%
