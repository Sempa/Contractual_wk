...
final_dataset %>%
  janitor::clean_names() %>%
  dplyr::select(all_of(desired_vars)) %>%
...
tbl_summary(#table_1,
  ...
  bold_labels()  %>%
  as_gt() %>%
  gt::tab_header('Table 1 (artcile): Patient presentation by CD4 count category')
...
# `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`
  ...
  bold_labels()  %>%
  as_gt() %>%
  gt::tab_header('Table 3 (article): Should compare Total AI with Non-AI patients')
...
PTB, EPTB,  `GE/c diff`, EPTB, PTB, AI
  ...
  bold_labels()  %>%
  as_gt() %>%
  gt::tab_header('Table 2 (article): Should compare Total PAI with SAI patients')
...
tbl_summary(#table_1,
  ...
  bold_labels() %>%
  as_gt() %>%
  gt::tab_header('Table 4 (article): Mortality: Non-AI vs AI')
table_4
dt04 <- table_3 %>%
