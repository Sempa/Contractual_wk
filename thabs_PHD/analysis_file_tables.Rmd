...
final_dataset <- read_excel("hypo_alldata_wide_labels.xls") %>%
  ...
  mutate(addisons_disease = ifelse(as.numeric(`Random cortisol result`) < 500, 1, 0),
         ...
         addisons_disease = ifelse(UIN...1 %in% c(392, 435, 478, 488, 496), 1, addisons_disease), #500 removed
         addisons_disease = ifelse(`Random cortisol result`<=274 & (is.na(`Synacthen: 0 minute cortisol result`) | is.na(`Synacthen: 30 minute cortisol result`)), 1, ifelse(UIN...1 == 500, 0, addisons_disease)),
         PAI = ifelse(`Random cortisol result`<=274 & (is.na(`Synacthen: 0 minute cortisol result`) | is.na(`Synacthen: 30 minute cortisol result`)) & (as.numeric(`ACTH result`) >= 64.7 & !is.na(`ACTH result`)) & PAI %in% c(0, NA), 1, ifelse(UIN...1 == 392, 1, PAI)),
         SAI = ifelse(`Random cortisol result`<=274 & (is.na(`Synacthen: 0 minute cortisol result`) | is.na(`Synacthen: 30 minute cortisol result`)) & (as.numeric(`ACTH result`) < 64.7) & SAI %in% c(0, NA), 1, SAI),
         total_AI = ifelse((PAI == 1 | SAI == 1), 1, 
                           ifelse((PAI != 1 & SAI != 1), 0, NA))
           ) %>%
  mutate(AI = ifelse(PAI == 1, 'PAI', 
                     ifelse(SAI == 1, 'SAI', ''))) %>%
  mutate(AI = ifelse(UIN...1 %in% c(435, 478, 488, 496), 'SAI',
                                   ifelse(UIN...1 %in% c(392), 'PAI', AI)),
         # AI = ifelse(addisons_disease == 0, 'No AI', AI),
  ...
  mutate(`Increased pigmentation of the skin` = as.logical(ifelse(`Increased pigmentation of the skin` == 'Yes', 1, 0)),
         ...
         incremental_cortisol = (abs(`Synacthen: 30 minute cortisol result` - `Synacthen: 0 minute cortisol result`)),
         incremental_cortisol_percent_increase = (abs(incremental_cortisol)/`Synacthen: 0 minute cortisol result`)*100
         ) %>%
...
`Any postural drop in blood pressure`, `Presence of anaemia`, mortality, incremental_cortisol_percent_increase,
Rifampicin, Fluconazole, Opiates, `ART exposure`, #`Kidney medication`, 
`Addisons disease`, #`Ethnicity, n(%)` = Ethnicity, 
...
`Duration of current illness, median (IQR) (days)` = `Duration of current illness`, 
`Weight loss`, `Random cortisol` = `Random cortisol result`, `Basal cortisol` = `Synacthen: 0 minute cortisol result`, `Stimulated cortisol` = `Synacthen: 30 minute cortisol result`, incremental_cortisol, incremental_cortisol_percent_increase, 
`Viral load (log10 Copies/mL)` = `log10 viral load`, 
