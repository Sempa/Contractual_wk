...
final_dataset <- read_excel("hypo_alldata_wide_labels 549.xlsx") %>%
...
# write.csv(final_dataset, 'opportunistic infections_549.csv')
# dt01 <- read_csv("opportunistic infections_549.csv") %>%
#   filter(!(`Hospital folder number` %in% read_csv("opportunistic infections.csv")$`Hospital folder number`))
# write.csv(dt01, 'opportunistic infections_updated_528_to_549.csv')
# dat <- final_dataset %>%
...
dt03 <- bind_rows(read_delim("list of drugs - updated 23 April24 TB & Fungal .csv", 
     delim = ";", escape_double = FALSE, trim_ws = TRUE) %>%
      dplyr::select(id, ART, tb, kidney, fungal),
    read_delim("list of drugs_additional_patients + CKD, TB & Fungal.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)) %>%
  dplyr::select(id, ART, tb = TB, kidney, fungal = Fungal) %>%
  group_by(id) %>%
  mutate(`ART exposure` = max(ifelse(is.na(ART), 0, ART), na.rm = T),
...
table_1 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `Viral load, log10 copies/ml`= `log10 viral load`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `ART exposure`, `Kidney medication`) %>%
tbl_summary(#table_1,
            missing = "no",
...
#   dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `log10 viral load`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `ART exposure`, `Kidney medication`) %>% #`Opportunistic infection present`, 
# tbl_summary(#table_1,
#             missing = "no",
#             by = `Gender, n(%)`,
...
table_12 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `log10 viral load`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `CD4 counts`, `ART exposure`, `Kidney medication`
    ) %>%# ,
tbl_summary(#table_1,
            missing = "no",
            by = `CD4 counts`,
  ...
  modify_header(label = "Variable") %>% # update the column header
  bold_labels()  %>%
  modify_caption('Table 3: Patient presentation by CD4 count category')
...
dt02 <- bind_rows(final_dataset %>%
  mutate(`AI status` = ifelse(addisons_disease==1, 'AI', 'No-AI'),
...
table_2 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `Viral load (log10 Copies/mL)` = `log10 viral load`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `Sodium mmol/L` = Sodium, `Potassium mmol/L` = Potassium, `Haemoglobin g/dL` = Haemoglobin, 
    ...
    `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection` = `Opportunistic infection present`, 
                # `3 followup_patientstatus`, `6 followup_patientstatus`, `12 followup_patientstatus`, 
    mortality, `ART exposure`, `Kidney medication`, `Addisons disease`) %>%
...
table_2_1 <- final_dataset %>%
  ...
  filter(AI != '') %>%
  dplyr::select(
    `Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `Viral load (log10 Copies/mL)` = `log10 viral load`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `AI`, `Sodium mmol/L` = Sodium, `Potassium mmol/L` = Potassium, `Haemoglobin g/dL` = Haemoglobin, 
    
   `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, 
   mortality, `ART exposure`, `Kidney medication`#, `total AI` = total_AI
                ) %>%
...
table_3 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment`, Gender, Ethnicity, `Duration of current illness`, `log10 viral load`, `Total CD4 count`, Sodium, Potassium, Haemoglobin, `White cell count`, `Lymphocyte count`, Neutrophils, `Addisons disease`, mortality, ttdeath, `Random cortisol result`, `Synacthen: 0 minute cortisol result`, `Synacthen: 30 minute cortisol result`, `ACTH result`, `BP (systolic)`, `BP (diastolic)`, discharged,
                ...
                Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Date of enrolment in study`, `HIV, when diagnosed`, AI_recoded, AI, total_AI, `ART exposure`, `Kidney medication`, incremental_cortisol) %>%
...
table_4 <- table_3 %>%
  ...
  dplyr::select(
    `Age at enrolment, median (IQR) (years)` = Age_at_enrolment, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = Duration_of_current_illness, `Random cortisol`, `Basal cortisol`, `Stimulated cortisol`, `ACTH`, `Weight loss`, `Viral load (log10 Copies/mL)`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count` = `CD4 count`, `White cell count X109` = White_cell_count, `Lymphocyte count X109` = Lymphocyte_count, Neutrophils, `Sodium mmol/L` = Sodium, `Potassium mmol/L`, `Haemoglobin g/dL` = Haemoglobin, 
    
    `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection`, 
               `ART exposure`, `Kidney medication`, incremental_cortisol, `Addisons disease` = Addisons_disease) %>%
  # mutate(`Addisons disease` = forcats::fct_rev(`Addisons disease`)) %>%
tbl_summary(#table_1,
            missing = "no",
            by = `Addisons disease`,
...
table_4_2 <- table_3 %>%
  ...
  dplyr::select(`mortality with Addisons disease`, `Age at enrolment median (IQR) (years)` = Age_at_enrolment, `Gender, n(%)` = Gender, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = HIV_duration, `Random cortisol`, `Basal cortisol`, `Stimulated cortisol`, `ACTH`, incremental_cortisol, `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Weight loss`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection`, 
                Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Viral load (log10 Copies/mL)`, `Total CD4 count` = `CD4 count`, `Sodium mmol/L` = Sodium, `Potassium mmol/L`, `Haemoglobin g/dL` = Haemoglobin, `White cell count X109` = White_cell_count, `Lymphocyte count X109` = Lymphocyte_count, `Addisons disease` = AI, `ART exposure`, `Kidney medication`) %>% #, `Addisons disease`
  # mutate(`mortality with Addisons disease` = forcats::fct_rev(`mortality with Addisons disease`)) %>%
  tbl_summary( # table_1,
    missing = "no",
...
univariate_mortality <- tbl_uvregression(
  table_3 %>%
    ...
    dplyr::select(Age_at_enrolment, Gender, Ethnicity, # HIV_duration,
      ...
      Viral_load = `Viral load (log10 Copies/mL)`, CD4_count = `CD4 count`, Sodium, Potassium = `Potassium mmol/L`, Haemoglobin, White_cell_count, Lymphocyte_count, `ART exposure`, `Kidney medication`, 
      Addisons_disease = `Addisons disease`, ttdeath, mortality
    ), # table_3,
  method = coxph,
  y = Surv(time = ttdeath, event = mortality),
  exponentiate = TRUE # ,
...
nr_imputations <- 5
dt <- table_3 %>%
  mutate(Weakness = as.numeric(Weakness),
         Ethnicity = as.numeric(Ethnicity),
         Tiredness = as.numeric(Tiredness),
         Poor_appetite = as.numeric(`Poor appetite`),
         Increased_pigmentation_of_the_skin = as.numeric(`Increased pigmentation of the skin`),
         Nausea = as.numeric(Nausea),
         Liking_for_salt = as.numeric(`Liking for salt`),
         Loss_of_consciousness = as.numeric(`Loss of consciousness`),
         Diarrhoea = as.numeric(Diarrhoea), 
         Dizziness = as.numeric(Dizziness),
         Shock = as.numeric(Shock), 
         Anorexia = as.numeric(Anorexia),
         Loss_of_axillary_and_pubic_hair = as.numeric(as.factor(`Loss of axillary and pubic hair, if female`)),
         Cryptococcus_neoformans = as.numeric(`Cryptococcus neoformans`),
         PCP = as.numeric(PCP),
         `Kidney medication` = as.numeric(`Kidney medication`)
         ) %>%
  dplyr::select(
    Age_at_enrolment, Ethnicity,
    Basal_cortisol = `Basal cortisol`, ACTH, BP_diastolic = `BP (diastolic)`,
    Weakness, Tiredness, Poor_appetite,
    Increased_pigmentation_of_the_skin,
    Nausea, Liking_for_salt, Loss_of_consciousness, Diarrhoea, Dizziness,
    Shock, Anorexia, Loss_of_axillary_and_pubic_hair,
    Cryptococcus_neoformans, PCP,
    Viral_load = `Viral load (log10 Copies/mL)`, CD4_count = `CD4 count`,
    Lymphocyte_count, `Kidney medication`, Addisons_disease, mortality, ttdeath
  )
imputedData <- mice::mice(
  data = data.frame(dt[, c(1:24)]),
  m = nr_imputations,
  maxit = 20,
...
# )
completeData_mort <- bind_cols(table_3 %>% 
                            dplyr::select(mortality, ttdeath, AI_recoded, 
                                          Gender, incremental_cortisol),#, Ethnicity, `ART exposure`, `Kidney medication`), 
  ...
  dplyr::select(mortality, ttdeath, AI, Gender,
                Ethnicity, Age_at_enrolment, Ethnicity, Basal_cortisol, ACTH, 
                BP_diastolic, Weakness, Tiredness, Poor_appetite, 
                Increased_pigmentation_of_the_skin, Nausea, Liking_for_salt, 
                Loss_of_consciousness, Diarrhoea, Dizziness, Shock, Anorexia, 
                Loss_of_axillary_and_pubic_hair, Cryptococcus_neoformans, PCP, 
                Viral_load, CD4_count, Lymphocyte_count, #incremental_cortisol, 
                Addisons_disease, Kidney_medication = `Kidney.medication`, incremental_cortisol) %>%
...
results <- rfe(completeData_mort[,c(3:28)], completeData_mort[[1]], rfeControl = control)
var_selected <- predictors(results)

model2 <- survival::coxph(Surv(time = ttdeath, event = mortality) ~ Viral_load + Cryptococcus_neoformans + Basal_cortisol + CD4_count,
                          data = completeData_mort %>%
                            # filter(as.character(Gender) != 'Unknown') %>%
                            mutate(Basal_cortisol = Basal_cortisol/50)
                          ) %>%#AI_recoded,# # + Dizziness,
  tbl_regression(exponentiate = TRUE) #%>%
  # modify_caption('COXPH model on effect of Addisons on mortality')
# model2
model3 <- glm(mortality ~ Viral_load + Cryptococcus_neoformans + Basal_cortisol + CD4_count,
              family = binomial(link = "logit"),
              data = completeData_mort %>%
                mutate(Basal_cortisol = Basal_cortisol/50)
              ) %>%
  tbl_regression(exponentiate = TRUE) #%>%
  # modify_caption('Logistic model on effect of Addisons on mortality')
# model3

tbl_merge(tbls = list(univariate_mortality, model2, model3),
          tab_spanner = c("**Bivariate**", "**multivariate - Cox PH**", "** multivariate - Logistic**"))

# adding increamental cortisol
dt <- completeData_mort %>%
  drop_na()
results <- rfe(dt[,c(3:29)], dt[[1]], rfeControl = control)
var_selected <- predictors(results)

model4 <- survival::coxph(Surv(time = ttdeath, event = mortality) ~ Viral_load + Cryptococcus_neoformans + BP_diastolic + Nausea + incremental_cortisol,
                          data = completeData_mort %>%
                            # filter(as.character(Gender) != 'Unknown') %>%
                            mutate(Basal_cortisol = Basal_cortisol/50)
                          ) %>%#AI_recoded,# # + Dizziness,
...
model5 <- glm(mortality ~ Viral_load + Cryptococcus_neoformans + BP_diastolic + Nausea + incremental_cortisol,
              family = binomial(link = "logit"),
              data = completeData_mort %>%
                mutate(Basal_cortisol = Basal_cortisol/50)
              ) %>%
  ...
  # modify_caption('Logistic model on effect of Addisons on mortality')
# model5
tbl_merge(tbls = list(univariate_mortality, model4, model5),
          tab_spanner = c("**Bivariate**", "**multivariate - Cox PH**", "** multivariate - Logistic**"))

x <- survival::coxph(Surv(time = t, event = event) ~ AI,
                  data = completeData_mort %>%
                      mutate(t = ifelse(ttdeath >3, 3, 0),
                             event = ifelse(ttdeath == 3, 1, 0)) %>%
                      mutate(Addisons = as.factor(ifelse(as.character(AI) == 'AI', "Yes", "No"))) %>%
                      dplyr::select(mortality, event, ttdeath, t, AI)
)
summary(survival::coxph(Surv(time = t, event = event) ~ AI,
                  data = completeData_mort %>%
                      mutate(t = ifelse(ttdeath >3, 3, 0),
                             event = ifelse(ttdeath == 3, 1, 0)) %>%
                      mutate(Addisons = as.factor(ifelse(as.character(AI) == 'AI', "Yes", "No"))) %>%
...
summary(survival::coxph(Surv(time = t, event = event) ~ AI,
                  data = completeData_mort %>%
                      mutate(t = ifelse(ttdeath >6, 6, 0),
                             event = ifelse(ttdeath == 6, 1, 0)) %>%
                      mutate(Addisons = as.factor(ifelse(as.character(AI) == 'AI', "Yes", "No"))) %>%
                      dplyr::select(mortality, event, ttdeath, t, AI)
))$coefficients[5]
...
summary(survival::coxph(Surv(time = t, event = event) ~ AI,
                  data = completeData_mort %>%
                      mutate(t = ifelse(ttdeath >12, 12, 0),
                             event = ifelse(ttdeath == 12, 1, 0)) %>%
                      mutate(Addisons = as.factor(ifelse(as.character(AI) == 'AI', "Yes", "No"))) %>%
                      dplyr::select(mortality, event, ttdeath, t, AI)
))$coefficients[5]
...
ggpar(survminer::ggsurvplot(km_fit,
  #conf.int = TRUE,
...
table_5 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment`, Gender, Ethnicity, `Duration of current illness`, `log10 viral load`, `Total CD4 count`, Sodium, Potassium, Haemoglobin, `White cell count`, `Lymphocyte count`, Neutrophils, `Addisons disease`, mortality, ttdeath, `Random cortisol result`, `Synacthen: 0 minute cortisol result`, `Synacthen: 30 minute cortisol result`, `ACTH result`, incremental_cortisol, `BP (systolic)`, `BP (diastolic)`, discharged,
                `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Weight loss`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection` = `Opportunistic infection present`,
                Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Date of enrolment in study`, `HIV, when diagnosed`, AI, `ART exposure`, `Kidney medication`, mortality) %>%
