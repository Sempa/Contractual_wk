...
table_5 <- table_3 %>%
  ...
  dplyr::select(
    `Age at enrolment, median (IQR) (years)`, `Female gender, n(%)`, `Duration of current illness, median (IQR) (days)`, 
`Weight loss`, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea,
`Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`,
`BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, `Any postural drop in blood pressure`, 
`Presence of anaemia`, `Sodium mmol/L`, `Potassium mmol/L`, `Haemoglobin g/dL`, Syphilis, Candida, HepB,
HSV, Pneumonia, PCP, EPTB, `Cryptococcus neoformans`, `Total CD4 count`, `Viral load (log10 Copies/mL)`,
`Random cortisol`, `Basal cortisol`, `Stimulated cortisol`, incremental_cortisol, incremental_cortisol_percent_increase, 
PTB, EPTB, `White cell count X109`,
Hypotension, Vomiting, `Liking for salt`, Hypoglycaemia, `ART exposure`, `Kidney medication`, ACTH, 
Rifampicin, Fluconazole, Opiates, `Addisons disease`, `Ethnicity, n(%)`, Tuberculosis, `Staph aureus`,
`Kaposis sarcoma`, Cytomegalovirus, `GE/c diff`, `Parvo B19`, `B menigitis`, `UTI / Leptospirosis`, `COVID-19`, 
Neurocysticercosis, `Lymphocyte count X109`, Neutrophils, mortality) %>%
  # mutate(`Addisons disease` = forcats::fct_rev(`Addisons disease`)) %>%
tbl_summary(#table_1,
            missing = "no",
            by = mortality, #`Addisons disease`,
            type = list(c("Duration of current illness, median (IQR) (days)", "Viral load (log10 Copies/mL)", "Lymphocyte count X109", "Neutrophils") ~ "continuous"),
          statistic = list(all_continuous() ~ "{median} ({p25}, {p75})"),
            digits = list(all_categorical() ~ c(0, 1)),
            ) %>%
  add_p() %>%
...
table_4_1 <- table_3 %>%
  ...
  dplyr::select(`mortality with Addisons disease`, `Age at enrolment median (IQR) (years)` = Age_at_enrolment, `Gender, n(%)` = Gender, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = HIV_duration, `Random cortisol`, `Basal cortisol`, `Stimulated cortisol`, `ACTH`, incremental_cortisol, incremental_cortisol_percent_increase, `BP (systolic)`, `BP (diastolic)`, `Heart rate`,
                Hypotension, Weakness, Tiredness, `Poor appetite`, `Weight loss`, `Increased pigmentation of the skin`, Nausea, Vomiting,
                `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and
                pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, 
                #`Presence of an opportunistic infection`, 
                Tuberculosis, PTB, EPTB,  `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV,
                HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`,
                `Neurocysticercosis`, `Viral load (log10 Copies/mL)`, `Total CD4 count` = `CD4 count`, `Sodium mmol/L` = Sodium, 
                `Potassium mmol/L`, `Haemoglobin g/dL` = Haemoglobin, `White cell count X109` = White_cell_count, 
                `Lymphocyte count X109` = Lymphocyte_count, AI, `Addisons disease`, `ART exposure`, `Kidney medication`, Rifampicin,
                Fluconazole, Opiates) %>%
...
table_4_2 <- table_3 %>%
  ...
  dplyr::select(`mortality with Addisons disease`, `Age at enrolment median (IQR) (years)` = Age_at_enrolment, `Gender, n(%)` = Gender,
                `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = HIV_duration, `Random cortisol`, 
                `Basal cortisol`, `Stimulated cortisol`, `ACTH`, incremental_cortisol, incremental_cortisol_percent_increase, 
                `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Weight loss`,
                `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`,
                Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`,
                `Presence of anaemia`, #`Presence of an opportunistic infection`, 
                Tuberculosis, PTB, EPTB,  `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV,
                HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`,
                `Neurocysticercosis`, `Viral load (log10 Copies/mL)`, `Total CD4 count` = `CD4 count`, `Sodium mmol/L` = Sodium, 
                `Potassium mmol/L`, `Haemoglobin g/dL` = Haemoglobin, `White cell count X109` = White_cell_count, 
                `Lymphocyte count X109` = Lymphocyte_count, `Addisons disease` = AI, `ART exposure`, `Kidney medication`, Rifampicin,
                Fluconazole, Opiates) %>% #, `Addisons disease`
  # mutate(`mortality with Addisons disease` = forcats::fct_rev(`mortality with Addisons disease`)) %>%
  tbl_summary( # table_1,
    missing = "no",
...
univariate_table <- tbl_uvregression(
          data_to_impute %>% 
            dplyr::select(mortality, selected_models$term, -record_id) %>% 
            mutate(Random_cortisol = Random_cortisol/50,
                   Basal_cortisol = Basal_cortisol/50,
                   BP_systolic = BP_systolic/10,
                   BP_diastolic = BP_diastolic/10,
...
lasso_results <- lapply(imp_list, function(dat) {
  ...
  x <- model.matrix(
    Surv(ttdeath, mortality) ~ .,
    dat
  )[ , -1]  # remove intercept

  y <- Surv(dat$ttdeath, dat$mortality)
...
final_formula <- as.formula(
  paste("Surv(ttdeath, mortality) ~", paste(stable_vars, collapse = " + "))
)

cox_fit <- with(
  imputedData,
  survival::coxph(
    survival::Surv(ttdeath, mortality) ~ ACTH + Opiates + Random_cortisol + Addisons_disease
  )
)

pooled_cox <- pool(cox_fit)
