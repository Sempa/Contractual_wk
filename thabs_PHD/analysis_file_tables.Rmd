...
final_dataset <- read_excel("hypo_alldata_wide_labels 549.xlsx") %>%
  mutate(
    ...
    total_AI = ifelse((as.numeric(`Random cortisol result`) < 500 & (PAI == 1 | SAI == 1)) | UIN...1 %in% c(392, 435, 478, 488, 496, 500), 1,
      ifelse(as.numeric(`Random cortisol result`) < 500 & (PAI != 1 & SAI != 1), 0, NA)
    ),
    addisons_disease = ifelse(is.na(total_AI), 0, total_AI)
  ) %>%
  mutate(
    AI = ifelse(PAI == 1, "PAI", ifelse(SAI == 1, "SAI", ifelse(UIN...1 %in% c(392, 435, 478, 488, 496, 500), 1, ""))),
    `Adrenal insufficiency` = ifelse(!is.na(addisons_disease), addisons_disease, 0)
  ) %>%
  mutate(
    `Increased pigmentation of the skin` = as.logical(ifelse(`Increased pigmentation of the skin` == "Yes", 1, 0)),
...
final_dataset <- final_dataset %>%
  left_join(bind_rows(
    ...
    mutate(
      Tuberculosis = as.logical(`Opportunistic infection  (choice=Tuberculosis)`),
    ...
    dplyr::select(`Hospital folder number`, Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, 
                  `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, Other, `GE/c diff`, `Parvo B19`, 
                  `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`
                  ) %>%
...
table_2 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment, median (IQR) (years)` = `Age at enrolment`, `Gender, n(%)` = `Gender`, `Ethnicity, n(%)` = Ethnicity, `Duration of current illness, median (IQR) (days)` = `Duration of current illness`, `Weight loss`, `Viral load (log10 Copies/mL)` = `log10 viral load`, Tuberculosis, PTB, EPTB,  `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Total CD4 count`, `White cell count X109` = `White cell count`, `Lymphocyte count X109` = `Lymphocyte count`, Neutrophils, `Sodium mmol/L` = Sodium, `Potassium mmol/L` = Potassium, `Haemoglobin g/dL` = Haemoglobin, 
    ...
    mortality, `ART exposure`, `Kidney medication`, `Kidney medication`, Rifampicin, Fluconazole, Opiates, `Addisons disease`) %>%
...
table_2_1 <- final_dataset %>%
  ...
  dplyr::select(
   ...
   mortality, `ART exposure`, `Kidney medication`, Rifampicin, Fluconazole, Opiates#, `total AI` = total_AI
                ) %>%
...
table_4 <- table_3 %>%
  ...
  dplyr::select(
    ...
    `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection`, 
               `ART exposure`, `Kidney medication`, incremental_cortisol, Rifampicin, Fluconazole, Opiates, `Addisons disease` = Addisons_disease) %>%
  # mutate(`Addisons disease` = forcats::fct_rev(`Addisons disease`)) %>%
tbl_summary(#table_1,
            missing = "no",
            by = `Addisons disease`,
...
dt <- table_3[, columns_to_impute] %>%
  dplyr::select(
    ...
    Random_cortisol = `Random cortisol`, ACTH, BP_systolic = `BP (systolic)`, BP_diastolic = `BP (diastolic)`, 
    ...
    Liking_for_salt = `Liking for salt`, Hypoglycaemia, Loss_of_consciousness = `Loss of consciousness`, 
    Diarrhoea, Dizziness, Shock, Anorexia, Any_postural_drop_in_blood_pressure = `Any postural drop in blood pressure`, 
    Presence_of_anaemia = `Presence of anaemia`, PTB, EPTB, Cryptococcus_neoformans = `Cryptococcus neoformans`, 
    Pneumonia, Staph_aureus = `Staph aureus`, Kaposis_sarcoma = `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, 
    Candida, GE_c_diff = `GE/c diff`, Parvo_B19 = `Parvo B19`, Syphilis, B_menigitis = `B menigitis`, 
    UTI_Leptospirosis = `UTI / Leptospirosis`, PCP, COVID_19 = `COVID-19`, Neurocysticercosis,
    ART_exposure = `ART exposure`, Kidney_medication = `Kidney medication`, Rifampicin, Fluconazole, Opiates) %>%# ttdeath, Loss of axillary and pubic hair, if female#Tuberculosis,
...
completeData_mort <- bind_cols(data_reduced %>% 
  ...
  #               Ethnicity, Age_at_enrolment, Ethnicity, ACTH, 
  ...
  #               CD4_count, #Lymphocyte_count, #incremental_cortisol, 
  #               Addisons_disease, Kidney_medication = `Kidney.medication`) %>%
...
# results <- rfe(completeData_mort[,c(2:53)], completeData_mort[[1]], rfeControl = control)
# var_selected <- predictors(results)

# Define function to perform automated multivariate linear regression
automated_regression <- function(data, outcome, covariates) {
  # Initialize best model
  # best_model <- NULL
  # best_adj_r_squared <- -Inf
  
  ########################
  # Initialize best models based on AIC and BIC
best_aic_model <- NULL
best_bic_model <- NULL
min_aic <- Inf
min_bic <- Inf
  ...
  # Loop through all possible combinations of covariates
  for (i in 1:length(covariates)) {
    for (combination in combn(covariates, i)) {
      # Create formula
      formula <- as.formula(paste(outcome, "~", paste(combination, collapse = " + ")))
  ...
  if (current_aic < min_aic) {
    min_aic <- current_aic
    best_aic_model <- current_model
  }
  
  if (current_bic < min_bic) {
    min_bic <- current_bic
    best_bic_model <- current_model
  }
      ###############################
...
# Return the best models
  return(list(Best_AIC_Model = summary(best_aic_model), 
       Best_BIC_Model = summary(best_bic_model)))
#####################################
}

# Define covariates
covariates <- names(completeData_mort[,2:53])

# Call the function
best_model <- automated_regression(completeData_mort, names(completeData_mort[,1]), covariates)
