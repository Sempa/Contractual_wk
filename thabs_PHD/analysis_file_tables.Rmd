...
final_dataset<- final_dataset %>%
  left_join(#bind_rows(
    read_excel("Updated OI's  05 March 2024.xlsx"#), 
                      # read_excel("opportunistic infections_updated_528_to_549.xlsx")
    ) %>% 
              replace(is.na(.), 0) %>%
...
dt03 <- read_delim("list of drugs - updated 23 April24.csv", 
...
table_12 <- final_dataset %>%
  ...
  mutate(`CD4 counts` = case_when(
    ...
    as.numeric(`Total CD4 count`) >34 & as.numeric(`Total CD4 count` <=66) ~ '33 - 66',
    as.numeric(`Total CD4 count`) > 67 & as.numeric(`Total CD4 count` <= 100) ~ '67 - 100'
  )) %>%
...
table_3 <- final_dataset %>%
  ...
  dplyr::select(`Age at enrolment`, Gender, Ethnicity, `Duration of current illness`, `log10 viral load`, `Total CD4 count`, Sodium, Potassium, Haemoglobin, `White cell count`, `Lymphocyte count`, Neutrophils, `Addisons disease`, mortality, ttdeath, `Random cortisol result`, `Synacthen: 0 minute cortisol result`, `Synacthen: 30 minute cortisol result`, `ACTH result`, `BP (systolic)`, `BP (diastolic)`, discharged,
                ...
                Tuberculosis, `Cryptococcus neoformans`, Pneumonia, `Staph aureus`, `Kaposis sarcoma`, Cytomegalovirus, HSV, HepB, Candida, `GE/c diff`, `Parvo B19`, `Syphilis`, `B menigitis`, `UTI / Leptospirosis`, PCP, `COVID-19`, `Neurocysticercosis`, `Date of enrolment in study`, `HIV, when diagnosed`, AI_recoded, AI, total_AI, `ART exposure`, `Kidney medication`) %>%
...
table_4 <- table_3 %>%
  ...
  dplyr::select(
    ...
    `BP (systolic)`, `BP (diastolic)`, `Heart rate`, Hypotension, Weakness, Tiredness, `Poor appetite`, `Increased pigmentation of the skin`, Nausea, Vomiting, `Liking for salt`, Hypoglycaemia, `Loss of consciousness`, Diarrhoea, Dizziness, Shock, Anorexia, `Loss of axillary and pubic hair, if female`, `Any postural drop in blood pressure`, `Presence of anaemia`, #`Presence of an opportunistic infection`, 
               `ART exposure`, `Kidney medication`, `Addisons disease` = Addisons_disease) %>%
  # mutate(`Addisons disease` = forcats::fct_rev(`Addisons disease`)) %>%
tbl_summary(#table_1,
            missing = "no",
            by = `Addisons disease`,
...
completeData_mort <- bind_cols(table_3 %>% 
                            dplyr::select(mortality, ttdeath, AI_recoded, 
                                          Gender),#, Ethnicity, `ART exposure`, `Kidney medication`), 
  ...
  dplyr::select(mortality, ttdeath, AI, Gender,
                ...
                Viral_load, CD4_count, Lymphocyte_count,# Kidney.medication, 
...
results <- rfe(completeData_mort[,c(1:24)], completeData_mort[[25]], rfeControl = control)
var_selected <- predictors(results)

model2 <- survival::coxph(Surv(time = ttdeath, event = mortality) ~ Viral_load + Basal_cortisol + Liking_for_salt + Ethnicity + Diarrhoea + Anorexia + Age_at_enrolment,
...
model2
model3 <- glm(mortality ~ Viral_load + Basal_cortisol + Liking_for_salt + Ethnicity + Diarrhoea + Anorexia + Age_at_enrolment,
